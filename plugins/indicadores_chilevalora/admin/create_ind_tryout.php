...
if(isset( $_GET['ind'] ) && is_numeric($_GET['ind'])){
	...
	foreach ($origins as $origin) {
		...
		if($types_indicators){
			...
			$html .=	'<hr>';
			$html .= 	'<table class="table data-table nopagination table-stripped">';
				$html .= 	'<thead>';
					$html .= 	'<tr>';
						$html .= 	'<td>';
							$html .= 	'ID';
					...
					$html .= 	'<tr>';			
						...
						$html .= 	'<td>';
							...
							$last_indicator_query = $wpdb->get_results("
								SELECT id_indicator, id_indicator_type, data, created_at 
								FROM cl_indicators
								WHERE status = 1 
